...
| D5 | Theme | **Same design system as the admin dashboard** (`client-v3/app/admin`): black background with a faint dot pattern, `#08080a` header with the Hult Prize logo, `#0e0e12` cards/tables (`border-white/15`, rounded-2xl/3xl), `#16161d` inputs and secondary surfaces, **white primary buttons** (the admin button variants are ported verbatim), mono uppercase micro-labels, and emerald/rose/amber/sky status colours. Hult pink `#f20089` stays as the brand accent (logo mark, option A, highlights). Font Google Sans. | User (updated 2026-09-24: "use same ui components and design theme as the admin dashboard") |
...
| D19 | Real-time | **Polling**: `GET /api/s/[code]/state` every 1000 ms while the tab is visible (10 s while hidden, so background tabs still load). Every response carries `serverNow`, and clients correct their clock offset from it. No SSE, which is safe on serverless or multi-instance hosting. | Default |
| D24 | DB load | Each server instance caches a per-session snapshot (session, questions, quiz teams, answers; eligible count cached 10 s) for 750 ms with **stale-while-revalidate**. Host actions, check-ins and taker changes hard-invalidate it; answers only mark it stale, so answer bursts don't make every poll reload. A code-to-session-id cache lets snapshot and answer loads run in one parallel round trip. Measured on a production build with 50 simulated teams: state p95 377 ms, answer p95 463 ms. | Default, tuned by load test |
