...
// ── Tab definition ──────────────────────────────────────────────────────────
const TABS = ["ALL", "FLAGSHIP", "COMPETITION", "WORKSHOP"] as const;
...
function parseEventDate(ev: PublicEvent) {
  const raw = ev.date;
  if (!raw) return null;
  const cleanStr = raw.includes("•") ? raw.split("•")[0].trim() : raw;
  const d = new Date(cleanStr);
...
function EventCard({ event, registeredTeam, onClick, onRegisterClick }: EventCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="event-card__body">
        ...
        <div className="event-card__meta">
          ...
          <div className="event-card__meta-row">
            ...
            <span>{event.venue || "Heritage Institute"}</span>
          </div>
          <div className="event-card__meta-row">
      ...
      {/* Card Branch Floral Image (Right) */}
...
export default function EventsHero({
  ...
  const filtered =
    activeTab === "ALL"
      ? events
      : events.filter((e) => e.tag?.toUpperCase().includes(activeTab));
            ...
            <div className="events-timeline-axis">
              ...
              {timelineEvents.map((ev, idx) => {
                ...
                const month = d
                  ...
                  : "NOV";
                const day = d ? d.getDate() : 14 + idx * 5;
                const isFirst = idx === 0;
